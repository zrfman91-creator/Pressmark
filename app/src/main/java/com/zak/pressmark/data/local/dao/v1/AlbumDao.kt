...
interface AlbumDao {
    ...
    @Query(
        """
    UPDATE ${Album.TABLE}
    SET ${Album.ARTWORK_PROVIDER} = 'discogs',
        ${Album.ARTWORK_PROVIDER_ITEM_ID} =
            CAST(${Album.DISCOGS_RELEASE_ID} AS TEXT)
    WHERE ${Album.ARTWORK_PROVIDER} IS NULL
      AND ${Album.DISCOGS_RELEASE_ID} IS NOT NULL
    """
    )
