...
class AppGraph(
    ...
    // --- Core Singletons ---
    private val database: AppDatabaseV2 by lazy(LazyThreadSafetyMode.SYNCHRONIZED) {
        DatabaseProviderV2.get(appContext)
    }
    ...
    // --- V2 Repository (Work -> Release -> Pressing -> Variant) ---
    val workRepositoryV2: WorkRepositoryV2 by lazy(LazyThreadSafetyMode.SYNCHRONIZED) {
        WorkRepositoryV2(database)
    }
    ...
    val releaseRepository: ReleaseRepository by lazy(LazyThreadSafetyMode.SYNCHRONIZED) {
        ReleaseRepository(
            ...
            workRepositoryV2 = workRepositoryV2,
        )
    }
